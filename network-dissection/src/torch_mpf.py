...
class Torch_LayerwiseFWRF(L.Module):
    def __init__(self, fmaps, nv=1, pre_nl=None, post_nl=None, dtype=np.float32):
        ...
        self.fmaps_shapes = [list(f.size()) for f in fmaps]  #the final feamaps are 2 (25,25)  (13,13)
        self.nf = np.sum([s[1] for s in self.fmaps_shapes])
        self.pre_nl  = pre_nl
        self.post_nl = post_nl
        self.nv = nv  # number of voxel
        ##nf is number of features
        self.rfs = [] #feceptive fields
        self.sm = L.Softmax(dim=1)
        for k,fm_rez in enumerate(self.fmaps_shapes):
            rf = L.Parameter(T.tensor(np.ones(shape=(self.nv, fm_rez[2], fm_rez[2]), dtype=dtype), requires_grad=True)) #build pooling feild for each voxel
            self.register_parameter('rf%d'%k, rf)   # distribute parameter for spatial pooling fields
            self.rfs += [rf,]
        #self.w  = L.Parameter(T.tensor(np.random.normal(0, 0.001, size=(self.nv, self.nf)).astype(dtype=dtype), requires_grad=True))
        #self.b  = L.Parameter(T.tensor(np.full(fill_value=0.0, shape=(self.nv,), dtype=dtype), requires_grad=True))
        self.w  = L.Parameter(T.tensor(np.random.normal(0, 0.01, size=(self.nv, self.nf)).astype(dtype=dtype), requires_grad=True)) #set weights for all feature and voxel
        self.b  = L.Parameter(T.tensor(np.random.normal(0, 0.01, size=(self.nv,)).astype(dtype=dtype), requires_grad=True)) #set bias for all feature and voxel
