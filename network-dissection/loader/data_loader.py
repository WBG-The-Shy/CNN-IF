...
class SegmentationData(AbstractSegmentation):
    ...
    def __init__(self, directory, categories=None, require_all=False):
        ...
        for cat in self.category:
            ...
            self.category_label[cat] = build_dense_label_array(
                    c_data, key='code')
        del self.category_label['color'][0]
        del self.category_label['texture'][0]
        del self.category_label['scene'][0]
        del self.category_label['object'][0]
        del self.category_label['part'][0]
        del self.category_label['material'][0]

        del self.label[0]

        self.category_map['color'] = np.delete(self.category_map['color'], [0])
        self.category_map['texture'] = np.delete(self.category_map['texture'], [0])
        self.category_map['scene'] = np.delete(self.category_map['scene'], [0])
        self.category_map['object'] = np.delete(self.category_map['object'], [0])
        self.category_map['part'] = np.delete(self.category_map['part'], [0])
        self.category_map['material'] = np.delete(self.category_map['material'], [0])

        self.category_unmap['color'] = np.delete(self.category_unmap['color'], [0])
        self.category_unmap['texture'] = np.delete(self.category_unmap['texture'], [0])
        self.category_unmap['scene'] = np.delete(self.category_unmap['scene'], [0])
        self.category_unmap['object'] = np.delete(self.category_unmap['object'], [0])
        self.category_unmap['part'] = np.delete(self.category_unmap['part'], [0])
        self.category_unmap['material'] = np.delete(self.category_unmap['material'], [0])
    ...
    def primary_categories_per_index(self, categories=None):
        ...
        for i in range(self.label_size(None)):
            maxcov, maxcat = max(
                    (self.coverage(cat, catmap[cat][i]-1)
