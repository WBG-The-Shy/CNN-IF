...
subject = 5
trn_subjects = [5]
trn_str = '5'                     # select the training dataset and subject
...
num_epochs = 50
holdout_size = 1200
model_params = torch.load("/AlexNet/V1_V2_V3_V4_FFA_EBA_RSC_VWFA/pretrained/16session_subj5_50epoch_50bs_lr=0.0010_hold=1200/AlexNet_Jul-21-2023_1614/model_params")
n_resample = 100
root_dir = os.getcwd() + '/'
...
output_dir = "/Training_Result/drwaimage/goal-driven/%s/%s/%s/%dsession_subj%s_%depoch_%dbs_lr=%.4f_hold=%d/%s_%s/"%(
...
#_fmaps_fn  have 8 extractors                                   image_data
...
best_params = model_params['params']
from src.torch_fwrf import  learn_params_ridge_regression, get_predictions, Torch_fwRF_voxel_block
...
np.savetxt("/Training_Result/AlexNet_goal_subj01_val.txt", val_cc, fmt="%s", delimiter=",")
