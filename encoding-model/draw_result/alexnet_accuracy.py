...
num_epochs = 50
holdout_size = 75
order_dir = "AlexNet_Jul-22-2023_1357"
n_resample = 100
root_dir = os.getcwd() + '/'
...
output_dir = "/Training_Result/drawimage/%s/%s/%s/%dsession_subj%s_%depoch_%dbs_lr=%.4f_hold=%d/%s_%s/"%(
...
GNet = alexnet_fmaps_filter(trn_stim_mean, trn_stim_std, fmask6, fmask7, fmask8).to(device)
GNet.load_state_dict(best_params['enc'])
fmaps = GNet(T.from_numpy(trn_stim_data[trn_subjects[0]][:1]).to(device))
