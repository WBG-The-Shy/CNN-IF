...
if feature_filter:
    from src.torch_feature_space import \
        filter_dnn_feature_maps  # to reduce the number of full convolutional layer features

    if not os.path.exists(filter_dir):
        alexnet = alexnet_fmaps(trn_stim_mean, trn_stim_std).to(device)
        os.makedirs(filter_dir)
        image = []
        for i in image_data:
            image.append(image_data[i])
        image_datas = np.concatenate([i for i in image], axis=0)
        alexnet, lmask, fmask, tuning_masks = filter_dnn_feature_maps(image_datas, alexnet, batch_size=feature_filter_size,
                                                                    fmap_max=feature_filter_num)
        np.savetxt(filter_dir + "feature_mask_layer6_%d.txt" % feature_filter_num, fmask[6], fmt="%s", delimiter=",")
        np.savetxt(filter_dir + "feature_mask_layer7_%d.txt" % feature_filter_num, fmask[7], fmt="%s", delimiter=",")
        np.savetxt(filter_dir + "feature_mask_layer8_%d.txt" % feature_filter_num, fmask[8], fmt="%s", delimiter=",")
        print("the feature filter of model:%s is finished, the feature filter number = %d" % (
        model_name, feature_filter_num))
        fmaps_new = alexnet(T.from_numpy(trn_stim_data[trn_subjects[0]][:1]).to(device))
        for k, _fm in enumerate(fmaps_new):
            print(_fm.size())
        del image_datas
        del image_data
        sys.exit(0)
    else:
        fmask6 = np.loadtxt(filter_dir + "feature_mask_layer6_%d.txt" % feature_filter_num, dtype=int, delimiter=",")
        fmask7 = np.loadtxt(filter_dir + "feature_mask_layer7_%d.txt" % feature_filter_num, dtype=int, delimiter=",")
        fmask8 = np.loadtxt(filter_dir + "feature_mask_layer8_%d.txt" % feature_filter_num, dtype=int, delimiter=",")
        alexnet = alexnet_fmaps_filter(trn_stim_mean, trn_stim_std, fmask6, fmask7, fmask8).to(device)
        del image_data
else:
    alexnet = alexnet_fmaps_filter(trn_stim_mean, trn_stim_std).to(device)

if pretrained:  # no matter filtered, they all need to load all parameters
    print('the model is pretrained on the ImageNet')
    checkpoint = alexnet.state_dict()
    Alexnet = torch.load('zoo/alexnet-owt-4df8aa71.pth')
    # Alexnet is already an orderdict
    para = []
    for i in Alexnet:
        para.append(Alexnet[i])
    j = 0
    for i in checkpoint:
        if i != 'mean' and i != 'std' and i != 'extractor.fmask6' and i != 'extractor.fmask7' and i != 'extractor.fmask8':  # to avoid the mean and std parameters
            checkpoint[i] = para[j]
            j = j + 1
    alexnet.load_state_dict(checkpoint)
# this has select the 512 max varance feature from f6,f7,f8
fmaps = alexnet(T.from_numpy(trn_stim_data[trn_subjects[0]][:1]).to(device))
...
param_count = 0
for w in alexnet.extractor.parameters():
...
optimizer_net = optim.Adam([
    ...
    {'params': alexnet.extractor.parameters()},
...
best_params, final_params, hold_cc_hist, hold_hist, trn_cc_hist, trn_hist, best_epoch, best_joint_cc_score = \
    learn_params_(output_dir, _training_fn, _holdout_fn, _pred_fn, alexnet, subject_fwrfs, subject_opts,
                  trn_stim_data, trn_voxel_data,
...
alexnet.load_state_dict(best_params['enc'])
torch.save(alexnet, output_dir + '%s_%s_%s_%dsession_subj%s_%depoch_%dbs_lr=%.4f_hold=%d.pth'%(
...
batch_size = 6
subject_val_cc = validation_(_pred_fn, alexnet, subject_fwrfs, val_stim_single_trial_data, val_voxel, batch_size)
...
# conduct resample 64 times     res = np.random.randint to get a new order of pred voxel
# n_resample = 100
subject_resample_val_cc = {}

for s in val_voxel_single_trial_data.keys():
    print('sampling subject %d' % s)
    subject_resample_val_cc[s] = cc_resampling_with_replacement(_pred_fn, alexnet, subject_fwrfs[s],
