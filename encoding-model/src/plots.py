...
def plot_fwrf_paper_compare(X, Y, threshold, xlim, ylim, cmap='Blues'):
    ...
    ax1 = g.ax_marg_x.hist(x[np.logical_and(mask, x < 0)], log=True, color=color1, bins=50,
    ...
    g.ax_marg_x.text(xlim[0] + 0.02, 50., '%.2f' % adv, horizontalalignment='left', fontsize=18, color=color1,
                     weight='bold')
    g.ax_marg_x.text(xlim[1] - 0.06, 50., '%.2f' % (1. - adv), horizontalalignment='left', fontsize=18, color=color2,
                     weight='bold')
