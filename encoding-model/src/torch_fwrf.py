...
class Torch_fwRF_voxel_block(nn.Module):
    ...
    def __init__(self, _fmaps_fn, params, _nonlinearity=None, input_shape=(1, 3, 227, 227), aperture=1.0):
        ...
        for k, n_pix in enumerate(self.fmaps_rez):
            pf = pnu.make_gaussian_mass_stack(models[:, 0], models[:, 1], models[:, 2], n_pix, size=aperture,
                                              dtype=np.float32)[2]
    ...
    def load_voxel_block(self, *params):
        ...
        for _pf, n_pix in zip(self.pfs, self.fmaps_rez):
            pf = pnu.make_gaussian_mass_stack(models[:, 0], models[:, 1], models[:, 2], n_pix, size=self.aperture,
                                              dtype=np.float32)[2]
    ...
    def forward(self, _fmaps):
        mst = []
        for _fm, _pf in zip(_fmaps, self.pfs):
            mst = mst + [torch.tensordot(_fm, _pf, dims=[[2, 3], [1, 2]])]
        _mst = torch.cat([torch.tensordot(_fm, _pf, dims=[[2, 3], [1, 2]]) for _fm, _pf in zip(_fmaps, self.pfs)],
                         dim=1)  # [#samples, #features, #voxels]
        ...
        r = []
        weight = list(torch.split(self.weights,[64,192,384,256,256,512,512,512],dim=1))
        for i in range(len(mst)):
            mst[i] = torch.transpose(torch.transpose(mst[i], 0, 2), 1, 2)
            r = r + [torch.squeeze(torch.bmm(mst[i], torch.unsqueeze(weight[i], 2))).t()]
        # self.weights = torch.ones((500, 2688), device=_mst.device(), dtype=torch.float32)# [#voxels, #samples, features]
        _r = torch.squeeze(torch.bmm(_mst, torch.unsqueeze(self.weights, 2))).t()  # [#samples, #voxels]
        if self.bias is not None:
            _r += torch.unsqueeze(self.bias, 0)
            for i in range(len(mst)):
                r[i] += torch.unsqueeze(self.bias, 0)
        return _r


def learn_params_ridge_regression(data, voxels, _fmaps_fn, models, lambdas, aperture=1.0, _nonlinearity=None,
                                  zscore=False, sample_batch_size=100, voxel_batch_size=100, holdout_size=100,
                                  shuffle=True, add_bias=False):
    """
    ...
    def _cofactor_fn(_x, lambdas):
        ...
        _f = torch.stack(
            [(torch.mm(torch.t(_x), _x) + torch.eye(_x.size()[1], device=device) * l).inverse() for l in lambdas],
            axis=0)  # [#lambdas, #feature, #feature]
    ...
    print('trn_size = %d (%.1f%%)' % (trn_size, float(trn_size) * 100 / len(voxels)))    #trn_size / len(voxel)
    print('dtype = %s' % dtype)
    print('device = %s' % device)
    print('---------------------------------------')
    # shuffle
    nt = len(data) # number of trils
    nm = len(models) # number of models
    nv = voxels.shape[1] # number of voxels
    ...
    mst_mean = None
    mst_std = None
    if zscore:   # zscore mean number x :  (x - mean) / std
    ...
    with torch.no_grad():
        for m, (x, y, sigma) in enumerate(models):
            ...
            _pfs = [_to_torch(pnu.make_gaussian_mass(x, y, sigma, n_pix, size=aperture, dtype=dtype)[2], device=device)
                    for n_pix in fmaps_rez] # make fwrf pooling fields
            for rt, rl in iterate_range(0, nt, sample_batch_size):
                _mst = torch.cat([torch.tensordot(_fm, _pf, dims=[[2, 3], [0, 1]]) for _fm, _pf in
                                  zip(_fmaps_fn(_to_torch(data[rt], device=device)), _pfs)],
                                 dim=1)  # [#samples, #features]  feature map * pooling field
    ...
    with torch.no_grad():
        for rv, lv in iterate_range(0, nv, voxel_batch_size):
            ...
            for rt, lt in iterate_range(0, nt, sample_batch_size):
                sys.stdout.write(
                    '\rsamples [%5d:%-5d] of %d, voxels [%6d:%-6d] of %d' % (rt[0], rt[-1], nt, rv[0], rv[-1], nv))
...
#
#     # device = next(_fmaps_fn.parameters()).device
#     # device = next(_fmaps_fn.parameters()).to(device)
#     trn_size = len(voxels) - holdout_size
#     assert trn_size>0, 'Training size needs to be greater than zero'
