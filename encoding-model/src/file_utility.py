...
### NIFTY STUFF ###
def load_mask_from_nii(mask_nii_file):   #read nii.gz  file
    return nib.load(mask_nii_file).get_fdata()
    
def view_data(vol_shape, idx_mask, data_vol, order='C', save_to=None):
    view_vol = np.ones(np.prod(vol_shape), dtype=np.float32) * np.nan     #np.prod  return all element *    np.ones(81*84*104)    the result is all nan
    img = nib.load("/home/mufan/VDisk1/Mufan/nsd/nsddata/ppdata/subj01/func1pt8mm/roi/floc-bodies.nii.gz")
    af = img.affine
    view_vol[idx_mask.astype('int').flatten()] = data_vol
    view_vol = view_vol.reshape(vol_shape, order=order)
    if save_to:
        nib.save(nib.Nifti1Image(view_vol, affine=af), save_to)  # 返回的是一个二维2的数组(N,M)，对角线的地方为1，其余的地方为0.
    return view_vol
###
...
def list_files(dir_path):
    fileNames = []
    for f in os.listdir(dir_path):   #得到路径path下的所有文件，返回list列表形式。     是返回的list列表顺序和path路径下的文件顺序是不一致的。
        if os.path.isfile(dir_path+f):   #判断某一对象(需提供绝对路径)是否为文件x`   os.path.isdir()：判断某一对象(需提供绝对路径)是否为目录
            fileNames += [dir_path+f,]
    return sorted(fileNames)   #sort  order
