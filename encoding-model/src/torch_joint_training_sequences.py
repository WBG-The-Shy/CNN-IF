...
def iterate_voxels(batch_params, voxel_params):
    ...
    if batchsize == totalsize:
        ...
        for i, startindex in enumerate(range(0, totalsize, batchsize)):
            ...
            sys.stdout.write('\r%-2s: %.1f %%: voxels [%6d:%-6d] of %d' % (
            seq[i % 4], float(i + 1) * 100 / n, shifted_index[0], shifted_index[-1], totalsize))
            for _p, p in zip(batch_params, voxel_params):
...
#################################################
def subject_pred_pass(_pred_fn, _ext, _con, x, batch_size):  # get the prediction result of voxel to stim
...
def subject_validation_pass(_pred_fn, _ext, _con, x, v,
                            batch_size):  # validation in each subject with the real response
    val_cc = np.zeros(shape=(v.shape[1]), dtype=v.dtype)
    val_pred = subject_pred_pass(_pred_fn, _ext, _con, x, batch_size)
    for i in range(v.shape[1]):
        val_cc[i] = np.corrcoef(v[:, i], val_pred[:, i])[0, 1]  # correlation
    return val_cc
...
def learn_params_(output_dir, _trn_fn, _hld_fn, _pred_fn, _ext, _cons, _opts, stims, voxels, num_epochs, batch_size,
                  holdout_size, trn_size=None, masks=None, randomize=False):
    ...
    hold_hist, trn_hist = [], []  # to record the loss history to draw a loss graph
    trn_cc_hist = {s: [] for s in voxels.keys()}  # draw training accuracy
    hold_cc_hist = {s: [] for s in voxels.keys()}  # draw testing accuracy
    val_cc_hist = {s: [] for s in voxels.keys()}  # draw validation accuracy
    best_joint_cc_score = float(0)
    best_params = {}
    best_epoch = 0

    # record the train para
    from torch.utils.tensorboard import SummaryWriter
    logger = SummaryWriter(log_dir=output_dir + './log')

    for epoch in range(num_epochs):
        ##
        ## Training pass for this subject
        ## begin to train
        f = open(output_dir + 'subject_trn_loss&cc_subject_hold_loss&cc.txt', "a")
        start_time = time.time()
        _ext.train()  # begin to train
        for s, _c in _cons.items():
            _c.train()

        trn_err = subject_training_pass(_trn_fn, _ext, _cons, _opts, trn_stims, trn_voxels, batch_size)
        logger.add_scalar('train_loss', trn_err, epoch + 1)

        joint_trn_cc = []
        for s, v in trn_voxels.items():
            train_cc = np.nan_to_num(subject_validation_pass(_pred_fn, _ext, _cons[s], trn_stims[s], v, batch_size))
            joint_trn_cc += [np.copy(train_cc), ]
            trn_cc_hist[s] += [np.copy(train_cc), ]

        joint_trn_cc_score = np.median(np.nan_to_num(np.concatenate(joint_trn_cc)))
        logger.add_scalar('train_accuracy', joint_trn_cc_score, epoch + 1)

        trn_hist += [trn_err, ]
        ##
        _ext.eval()  # begin to test and eval the grad
        for s, _c in _cons.items():
            _c.eval()
        hold_err = subject_holdout_pass(_hld_fn, _ext, _cons, hld_stims, hld_voxels, batch_size)
        logger.add_scalar('hold_loss', hold_err, epoch + 1)
        ...
        f.write("\n  training loss:               {:.6f}".format(trn_err))
        f.write("\n  holdout loss (batch):        {:.6f}".format(hold_err))
        ...
        for s, v in hld_voxels.items():
            ...
            f.write("\n  Subject {}: median (max) validation accuracy = {:.3f} ({:.3f})".format(s, np.median(
                np.nan_to_num(hold_cc)), np.max(np.nan_to_num(hold_cc))))

            print("  Subject {}: median (max) validation accuracy = {:.3f} ({:.3f})".format(s, np.median(
                np.nan_to_num(hold_cc)), np.max(np.nan_to_num(hold_cc))))
        ##
        ...
        logger.add_scalar('hold_accuracy', joint_cc_score, epoch + 1)
        if joint_cc_score > best_joint_cc_score:
            f.write("\n** Saving params with joint score = {:.3f} **".format(joint_cc_score))
    ...
    return best_params, final_params, hold_cc_hist, hold_hist, trn_cc_hist, trn_hist, best_epoch, best_joint_cc_score


def validation_(_pred_fn, _ext, _cons, stims, voxels, batch_size):  # get the parameter of encoder and couple model
    val_ccs = {}
    _ext.eval()
...
def cc_resampling_with_replacement(_pred_fn, _ext, _con, x, v, batch_size, n_resample=1):
    pred = subject_pred_pass(_pred_fn, _ext, _con, x, batch_size)
